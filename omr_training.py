...
# Parameterization
img_height = 128
params = omr_model.default_model_params(img_height,primus.vocabulary_size)
max_epochs = 64000
dropout = 0.5 # reduce overfit 

# Model
inputs, seq_len, targets, decoded, loss, rnn_keep_prob = omr_model.ctc_crnn(params) 
...
for epoch in range(max_epochs):
    ...
    _, loss_value = sess.run([train_opt, loss],
                             feed_dict={
                                ...
                                targets: omr_utils.sparse_tuple_from(batch['targets']),
    ...
    if epoch % 100 == 0:
        ...
        while val_idx < validation_size:
            ...
            for i in range(len(str_predictions)):
                ed = omr_utils.edit_distance(str_predictions[i], validation_batch['targets'][val_idx+i])
                val_ed = val_ed + ed
