...
class CTC_PriMuS:
    ...
    def __init__(self, corpus_dirpath, corpus_filepath, dictionary_path, semantic, val_split = 0.0):
        ...
        corpus_list = next(os.walk(corpus_dirpath))[1]
        cut_index = len(corpus_list) // 2
        corpus_list = corpus_list[cut_index:]
        self.current_idx = 0
    ...
    def nextBatch(self, params):
        ...
        for _ in range(params['batch_size']):
            ...
            sample_img = omr_utils.resize(sample_img,height)
            images.append(omr_utils.normalize(sample_img))

            # GROUND TRUTH
            sample_full_filepath = sample_fullpath + '.semantic'
            sample_gt_file = open(sample_full_filepath, 'r')
            sample_gt_plain = sample_gt_file.readline().rstrip().split(omr_utils.word_separator())
